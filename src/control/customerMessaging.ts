 ...
 *  - `greeting`: the one answer to a bare "Hi"/"Hello" that opens a fresh conversation with no case in it.
 */
export const CUSTOMER_MESSAGING_ENABLED = false;

/** Message kinds allowed through while the switch above is `false`. */
export const ENABLED_CUSTOMER_MESSAGES: ReadonlySet<string> = new Set<string>(['evidence_request', 'payment_confirmed', 'greeting']);
