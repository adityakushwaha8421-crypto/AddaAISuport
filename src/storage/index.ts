...
export async function createStore(env: Env, log: Logger): Promise<Store> {
  if (env.STORE === 'memory') {
    log.warn({ requestsFile: env.REQUESTS_STATE_FILE, usersFile: env.USERS_STATE_FILE }, 'Using in-memory store: messages are lost on restart; customers, the evidence-request ledger and the ON/OFF switch are kept on disk');
    return new MemoryStore({ requestsFile: env.REQUESTS_STATE_FILE, usersFile: env.USERS_STATE_FILE });
  }
